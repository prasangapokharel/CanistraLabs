#!/usr/bin/env python3
"""
Production-Ready User Flow Test - Fixed Version
===============================================
...
<body>
    ...
    def test_step_1_register(self) -> bool:
        ...
        payload = {
            "username": f"produser_{int(time.time())}",
            "email": self.user_email,
            "password": "SecurePassword123!",
        }

        try:
            response = self.session.post(f"{BASE_URL}/api/v1/auth/signup", json=payload)
            if response.status_code == 201:
                print(f"✅ User registered successfully: {self.user_email}")
                return True
            else:
    ...
    def test_step_4_deploy_project(self) -> bool:
        ...
        try:
            ...
            # Send deployment request with empty body (use project's existing code)
            deploy_payload = {"code_content": None, "force": False}
    ...
    def test_step_6_unique_canister_verification(self) -> bool:
        ...
        try:
            ...
            deploy_payload_2 = {"code_content": None, "force": False}
            response = self.session.post(
                f"{BASE_URL}/api/v1/deployments/projects/{project_2_id}/deploy",
                json=deploy_payload_2,
                timeout=180,
            )
